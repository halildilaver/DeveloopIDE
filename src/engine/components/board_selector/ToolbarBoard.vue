<template>
    <div>
        ...
        <v-dialog v-model="boardDialog" max-width="80%" max-height="81%" scrollable persistent>
            <v-card>
                <v-card-title>
                    <span class="headline">Seçili kart : {{ this.$global.board.board_info.title }}</span>
                    ...
                    <v-menu bottom left>
                        ...
                        <v-list>
                            <v-list-tile @click="changeOrder('recommended')">
                                <v-list-tile-title>Önerilenlere göre sırala</v-list-tile-title>
                            </v-list-tile>
                            <v-list-tile @click="changeOrder('name')">
                                <v-list-tile-title>İsime göre sırala</v-list-tile-title>
                            </v-list-tile>
                        </v-list>
                    </v-menu>
                </v-card-title>
                ...
                <v-card-actions>
                    ...
                    <v-btn class="btn-danger" flat @click.native="boardDialog = false">Kapat</v-btn>
                </v-card-actions>
            </v-card>
        </v-dialog>
    </div>
    ...
    methods: {
      ...
      orderBoardBy(obj, orderBy, defaultName) {
        ...
        if (orderBy === "recommended") {
          ...
          obj.forEach(el => {
            let key =
              el.title.toLowerCase().includes("develoop") ||
              el.recommended === "develoop"
                ? "Önerilenler"
                : defaultName;
            if (!(key in sorted)) {
              sorted[key] = [];
