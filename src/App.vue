<template>
    <div id="appRoot">
        ...
        <v-dialog persistent v-model="firstUseDialog" max-width="450">
            <v-card>
                <v-card-title>
                    <span class="headline">DeveloopIDE'ye Hoşgeldiniz</span>
                </v-card-title>
                <v-card-text>
                    <p class="title">
                        DeveloopIDE bir Kivvi R&D ürünüdür.
                    </p>
                    <p class="subtitle">
                        Detaylı bilgi için lütfen https://www.kivvi.com.tr adresini ziyaret ediniz.
                        <br/><br/>
                    </p>
                </v-card-text>
                <v-card-actions>
                    ...
                    <v-tooltip bottom>
                        <template v-slot:activator="{ on }">
                            <v-btn v-on="on" color="blue darken-1" flat @click="skipTour">Tamam</v-btn>
                        </template>
                    </v-tooltip>
                </v-card-actions>
            </v-card>
  ...
  export default {
    ...
    async created() {
      ...
      electron.ipcRenderer.on("help-tour", () => {
        this.$global.setting.firstUse = false; //true
        window.getApp.firstUseDialog = false;   //true halil
      });
